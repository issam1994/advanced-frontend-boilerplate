...
function livePreview(done) {
  php.server(
    ...
    function () {
      browserSync.init({
        proxy: `localhost:${options.config.port}`,
      });
    }
  );
  done();
...
//Development Tasks
function devHTML() {
  return src(`${options.paths.src.base}/**/*.php`).pipe(
    dest(options.paths.dist.base)
  );
}

function devStyles() {
  return src(`${options.paths.src.css}/main.dev.css`)
...
function watchFiles() {
  watch(
    `${options.paths.src.base}/**/*.php`,
    series(devHTML, devStyles, previewReload)
  );
  watch(
    [options.config.tailwindjs, `${options.paths.src.css}/modules/*.css`],
...
exports.prod = series(
  ...
  parallel(devStyles, prodStyles, prodScripts, prodImages, prodFonts, prodHTML), //Run All tasks in parallel
  buildFinish
);
